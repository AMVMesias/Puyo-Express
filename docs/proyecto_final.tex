...
\toprule
\textbf{Activo} & \textbf{Ubicación y Endpoints} & \textbf{Criticidad} & \textbf{Protección} \\
\midrule
Contraseñas de Usuarios & \parbox[t]{4.8cm}{DB (\texttt{users.password}) \\ \texttt{/api/auth/*}} & Crítico & Hash BCrypt (costo 12) \\ \addlinespace
Teléfono y Dirección & \parbox[t]{4.8cm}{DB (\texttt{orders.customer\_*}) \\ \texttt{/api/orders/*}} & Confidencial & AES-256-GCM por campo \\ \addlinespace
Correo electrónico & \parbox[t]{4.8cm}{DB (\texttt{users.email}) \\ \texttt{/api/auth/*}, \texttt{/api/users/*}} & Confidencial & Cifrado de volumen y TLS \\ \addlinespace
Claves Secretas (JWT, AES) & \parbox[t]{4.8cm}{Docker secrets \\ Interno en backend} & Crítico & 256 bits aleatorios, sin Git \\ \addlinespace
Tokens JWT (Sesión) & \parbox[t]{4.8cm}{Cookies del Cliente \\ \texttt{/api/*}} & Crítico & HttpOnly, SameSite, Secure \\ \addlinespace
Credenciales DB & \parbox[t]{4.8cm}{Docker secrets \\ Interno} & Crítico & Gestión manual fuera de Git \\ \addlinespace
Logs de Auditoría & \parbox[t]{4.8cm}{stdout del contenedor \\ \texttt{/api/*}} & Confidencial & Sanitación (Sin payloads ni PII) \\
\bottomrule
\end{tabular}
\end{table}
